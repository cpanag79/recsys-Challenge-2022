...
function [PR,PR_V] = getPropA_A_1(SES_TR,MTRS,MTRP,TRAIN2,CI_B,CI_V,UseSparse)
...
for i=1:length(TRAIN2)
    ...
    for j=2:length(views)
        PR(views(j),views(j-1)) = PR(views(j),views(j-1))+1;
        NumView(views(j-1)) = NumView(views(j-1))+1;
    end
